class Php7
  def Php7.configure(config, settings)
    ...
    if settings['networking'][0]['public']
      config.vm.network :public_network, ip: settings['networking'][0]["ip"] ||= "192.168.7.6"
    end
    ...
    if settings['folders'].kind_of?(Array)
      settings["folders"].each do |folder|
        if folder["type"] == "nfs"
          ...
          config.vm.synced_folder folder["map"], folder["to"], type: folder["type"] ||= nil , owner: folder["owner"] ||= "www-data", group: folder["group"] ||= "www-data"
        end
      end
    end
    ...
    if settings['databases'].kind_of?(Array)
      settings["databases"].each do |db|
          config.vm.provision "shell" do |s|
              s.path = "./vagrant_config/scripts/create-mysql.sh"
              s.args = [db]
          end
      end
    end
